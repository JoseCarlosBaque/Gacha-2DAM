...
public class EditarServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)  throws ServletException, IOException {
        ...
        String nvUser  = request.getParameter("nvUser");
        String nvClave  = request.getParameter("nvClave");
        String contra  = request.getParameter("contra");
        if (contra != null && (contra.equals(user.getClave()))) {
            response.sendRedirect("editar.jsp");
        } else if (request.getParameter("enviar") != null) {
            user.setUsuario(nvUser);
            user.setClave(nvClave);
